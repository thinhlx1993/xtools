...
const mapUserTweet = (entry) => {
  try {
    ...
    const tweetResultsTypeName =
      entry.content.itemContent.tweet_results.result.__typename;
    const isAds =
      !!entry.content.itemContent.promotedMetadata?.advertiser_results;
    const isRePost =
    ...
    const limitedActions = (
      entry.content.itemContent.tweet_results.result?.limitedActionResults
        ?.limited_actions || []
        ...
        if (unifiedCardBindingValue) {
          ...
          switch (unifiedCardBindingObject.type) {
            case UNIFIED_CARD_TYPE.videoWebsite:
              Object.values(unifiedCardBindingObject.media_entities).forEach(
                (media) => {
                  switch (media.type) {
...
const mapTweetDetail = (entry) => {
  ...
  if (
    entry.content.entryType === ENTRY_TYPE.item &&
  ...
  return contentItems
    .map((contentItem, index) => {
      try {
        ...
        const tweetResultsTypeName =
          contentItem.item.itemContent.tweet_results.result.__typename;
        const isAds =
          !!contentItem.item.itemContent.promotedMetadata?.advertiser_results;
        const isReply =
          !!contentItem.item.itemContent.tweet_results.result?.legacy
            ?.retweeted_status_result ||
          !!contentItem.item.itemContent.tweet_results.result?.tweet?.legacy
            .retweeted_status_result;
          ...
          if (tweet.card?.legacy?.binding_values?.length) {
            ...
            if (unifiedCardBindingValue) {
              ...
              switch (unifiedCardBindingObject.type) {
                case UNIFIED_CARD_TYPE.videoWebsite:
                  Object.values(
                    unifiedCardBindingObject.media_entities
                  ).forEach((media) => {
                    switch (media.type) {
                      case TWEET_MEDIA_TYPE.video:
                        unifiedCard.media.push({
              ...
              Object.values(
                unifiedCardBindingObject.destination_objects
